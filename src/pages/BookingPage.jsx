...
const TABLES = [
  {
    id: "indoor-cozy",
    label: "Indoor Cozy",
    description: "Intimate corner booth with warm lighting",
    icon: "🕯️",
    max: 4,
  },
  {
    ...
    description: "Open-air dining with garden views",
    icon: "🌿",
    max: 8,
  },
  {
  ...
  },
  {
    ...
    description: "Watch the chefs work their magic",
    icon: "👨‍🍳",
    max: 6,
  },
];
      ...
      <div className="min-h-screen pt-20 flex items-center justify-center px-6 bg-clay-50">
        ...
        >
          ...
          <p className="text-gray-500 mb-6">
            Your table for <strong>{form.guests} guests</strong> is reserved on{" "}
            <strong>{form.date}</strong> at <strong>{form.time}</strong>. We'll
            send a confirmation to <strong>{form.email}</strong>.
          </p>
          ...
          <button
            onClick={() => {
              setConfirmed(false);
              setStep(0);
              setForm((f) => ({ ...f, date: "", time: "", tableType: "" }));
            }}
      ...
      <div className="hero-gradient py-12 px-6 relative overflow-hidden noise-overlay">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-white/60 text-sm">
            Secure your perfect dining moment
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
          ...
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center">
              <div className={`flex flex-col items-center`}>
                <motion.div
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                    i < step
                      ...
                      : i === step
                        ? "bg-saffron-400 text-forest-900 shadow-amber"
                ...
                <span
                  className={`text-[10px] mt-1 font-medium ${i === step ? "text-forest-700" : "text-gray-400"}`}
                >
                  {s}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <div
                  className={`w-16 h-0.5 mx-1 mb-4 transition-colors ${i < step ? "bg-forest-700" : "bg-gray-200"}`}
                />
              )}
            </div>
          ))}
        ...
        <AnimatePresence mode="wait">
          ...
          >
              ...
              <div className="space-y-5">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-1.5 block flex items-center gap-1.5">
                    <Calendar size={14} className="text-forest-600" /> Select
                    Date
                  </label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-2 block flex items-center gap-1.5">
                    <Users size={14} className="text-forest-600" /> Number of
                    Guests
                  </label>
                  <div className="flex items-center gap-4">
                    <button
                      onClick={() =>
                        set("guests", Math.max(1, form.guests - 1))
                      }
                      className="w-10 h-10 border border-gray-200 rounded-full flex items-center justify-center text-forest-700 hover:border-forest-500 transition-colors text-xl"
                    >
                      -
                    </button>
                    <span className="font-display font-bold text-2xl text-forest-900 w-8 text-center">
                      {form.guests}
                    </span>
                    <button
                      onClick={() =>
                        set("guests", Math.min(12, form.guests + 1))
                      }
                      className="w-10 h-10 bg-forest-700 text-white rounded-full flex items-center justify-center hover:bg-forest-600 transition-colors text-xl"
                    >
                      +
                    </button>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {TABLES.filter((t) => t.max >= form.guests).map((table) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-forest-600 font-medium mt-2">
                        Up to {table.max} guests
                      </p>
                    </motion.button>
                  ))}
                </div>
            ...
            {step === 3 && (
              <div>
                <h2 className="font-display font-bold text-xl text-forest-900 mb-5">
                  Confirm Reservation
                </h2>
                <div className="space-y-3 mb-6">
                  {[
                    ...
                    [
                      "🪑 Table",
                      TABLES.find((t) => t.id === form.tableType)?.label,
                    ],
                    ["✨ Occasion", form.special],
                    ...
                    >
                      <span className="text-gray-500">{label}</span>
                      <span className="font-medium text-forest-900 max-w-[55%] text-right">
                        {val}
                      </span>
                    </div>
                  ))}
