...
import { Minus, Plus, Trash2, ShoppingBag, ArrowRight, Tag, ChevronRight, Truck, Shield } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen pt-28 flex flex-col items-center justify-center px-6">
        <motion.div className="text-center" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h2 className="font-display text-2xl font-bold text-forest-900 mb-2">Your cart is empty</h2>
  ...
  return (
    <div className="min-h-screen pt-20 pb-12 bg-clay-50">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        >
          <div>
            <h1 className="font-display text-3xl font-bold text-forest-900">Your Cart</h1>
            <p className="text-gray-500 text-sm mt-1">{cart.length} item{cart.length > 1 ? 's' : ''} in your order</p>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-3">
              ...
              {cart.map((item, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display font-semibold text-forest-900 text-sm">{item.name}</h3>
                    <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{item.description}</p>
                    <p className="font-bold text-forest-800 text-base mt-1.5">${(item.price * item.qty).toFixed(2)}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="font-bold w-6 text-center text-forest-900">{item.qty}</span>
                    <motion.button
                      onClick={() => updateQty(item.id, item.qty + 1)}
            ...
            {/* Add more */}
            <Link to="/menu" className="flex items-center justify-center gap-2 border-2 border-dashed border-gray-200 rounded-2xl py-5 text-sm text-gray-400 hover:border-forest-400 hover:text-forest-600 transition-colors group">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mt-4">
              {[
                ...
                <div key={title} className="flex items-center gap-3 bg-white rounded-2xl p-4 border border-gray-100">
                  ...
                  <div>
                    <p className="font-semibold text-sm text-forest-900">{title}</p>
          ...
          >
            <div className="card p-5">
              <h2 className="font-display font-bold text-forest-900 mb-5">Order Summary</h2>

              {/* Promo code */}
              <div className="mb-4">
                <label className="text-xs text-gray-500 font-medium mb-1.5 block">Promo Code</label>
                <div className="flex gap-2">
                  ...
                  <button onClick={applyPromo} className="px-4 py-2 bg-forest-700 text-white text-sm rounded-xl font-medium hover:bg-forest-600 transition-colors">
                    Apply
                  </button>
                </div>
                {promoError && <p className="text-red-500 text-xs mt-1">{promoError}</p>}
                {appliedPromo && (
                  <motion.p className="text-forest-600 text-xs mt-1 font-medium" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between text-gray-600">
                  <span>Delivery</span>
                  <span className={delivery === 0 ? 'text-forest-600 font-medium' : ''}>{delivery === 0 ? 'FREE' : `$${delivery.toFixed(2)}`}</span>
                </div>
                {discount > 0 && (
                  <motion.div
            ...
            {subtotal < 25 && (
              ...
              >
                ...
                <p className="text-xs text-amber-700">
                  Add <strong>${(25 - subtotal).toFixed(2)}</strong> more to get <strong>FREE delivery!</strong>
                </p>
              </motion.div>
            )}
          </motion.div>
